...
//NEXT EDITING
    ...
    if($star > 80 && $star <=100)
    {
      ...
      $user_check_query = "SELECT * FROM inception WHERE id='$id' OR 'slot=$slot' LIMIT 1";
  $result = mysqli_query($db, $user_check_query);
  $user = mysqli_fetch_assoc($result);

if (count($errors) == 0) {
  $query="INSERT INTO inception(username,comment,rating) values('$username','$comment','$star')";
    mysqli_query($db, $query);


    $_SESSION['success'] = "reviewed";
